...
const Switcher: React.FC<SwitcherProps> = ({
  ...
  // Sincroniza o estado interno com a prop 'checked' se for um componente controlado
  useEffect(() => {
    setIsChecked(checked);
  }, [checked]);

  // Gerar um ID único para o input e label
    ...
    <div className={customClasses}>
      ...
      >
        <div className="relative">
          ...
          />
          <div
            className={`block h-8 w-14 rounded-full bg-gray-3 dark:bg-[#5A616B]`}
          ></div>
          <div
            className={`dot absolute left-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow-switch-1 transition ${
              isChecked && 'translate-x-full !bg-primary dark:!bg-white'
            }`}
          >
        ...
        {label && (
          <span className="ml-2 text-body-sm font-medium">{label}</span>
        )}
      </label>
    </div>
  );
